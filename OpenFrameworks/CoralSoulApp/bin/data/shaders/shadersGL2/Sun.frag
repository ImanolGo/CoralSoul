...
float noise( vec3 x )
{
    ...
    vec2 rg = texture2DRect( iChannel0, (uv + 0.5)/256.0 ).yx;
    return mix( rg.x, rg.y, f.z );
}

float map(vec3 p) {
    vec3 q = p + 0.2*vec3(3.0, 0.3, 5.0)*mod(iTime*speed,3600.0)*2.0;
    float n = 0.0, f = 0.5;
...
void main( void )
{
    ...
    vec3 col = tr * shadeBg(-d, gl_FragCoord.xy);
    
    vec4 noise = (texture2DRect(iChannel0, mod(gl_FragCoord.xy/256.0, 1.0))-0.5) / 64.0;
    gl_FragColor = pow(vec4( noise.rgb + (1.0 - exp(-1.3 * col.rgb)), 1.0 ), vec4(1.3));
}